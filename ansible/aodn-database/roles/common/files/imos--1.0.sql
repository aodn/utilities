...
DROP AGGREGATE IF EXISTS make_trajectory(Geometry);
...
CREATE OR REPLACE FUNCTION is_valid_point(point geometry)
RETURNS boolean AS
$$
BEGIN
...
CREATE OR REPLACE FUNCTION make_point_or_shortest_line(pointa geometry, pointb geometry)
RETURNS geometry AS
$$
BEGIN
...
CREATE OR REPLACE FUNCTION st_createfishnet(p_nrow integer, p_ncol integer, p_xsize double precision, p_ysize double precision, p_x0 double precision DEFAULT 0, p_y0 double precision DEFAULT 0, p_srid integer DEFAULT 4326, OUT "row" integer, OUT col integer, OUT cell geometry)
...
CREATE OR REPLACE FUNCTION create_grid_cells(p_resolution double precision, OUT "row" integer, OUT col integer, OUT cell geometry)
  RETURNS SETOF record AS
$BODY$
BEGIN
    RETURN QUERY (
...
CREATE OR REPLACE FUNCTION BoundingPolygon(p_schema_name text, p_table_name text, p_column_name text, p_resolution double precision)
    RETURNS geometry AS
$BODY$
DECLARE
    result text;
...
CREATE OR REPLACE FUNCTION add_id_to_polygons( p_gml text )
  RETURNS text AS
$BODY$
DECLARE
    parts text[];
...
CREATE OR REPLACE FUNCTION BoundingPolygonAsGml3(p_schema_name text, p_table_name text, p_column_name text, p_resolution double precision)
    RETURNS text AS
$BODY$
DECLARE
...
CREATE OR REPLACE FUNCTION drop_objects_in_schema( schema text ) returns void
LANGUAGE plpgsql volatile
AS $$
    BEGIN
