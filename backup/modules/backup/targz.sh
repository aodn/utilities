...
backup() {
	...
	logger_info "Creating tar.gz archive with parameters '$@'"
	(cd / && $sudo tar -czf $_BACKUP_DEST/$_BACKUP_OBJECT_NAME.tar.gz "$@")
}
...
restore() {
	...
	(cd / && $sudo tar -xzf $_BACKUP_DEST/$_BACKUP_OBJECT_NAME.tar.gz)
}
